...
write_step "Stopping user service if it exists"
if systemctl --user list-unit-files | grep -Fq "${SERVICE_NAME}.service"; then
...
write_step "Writing user systemd service"
mkdir -p "$SYSTEMD_DIR"
...
[Service]
Type=simple
ExecStart=$NANOBOT_CMD gateway --config $CONFIG_PATH
Restart=always
RestartSec=5
...
if [ "$SKIP_SERVICE_RESTART" != "1" ]; then
  write_step "Restarting user service"
